...
def create_and_load_table():
    ...
    try:
        # 🐛 Solución: Se especifica la codificación 'latin1' para evitar errores
        ubicaciones_df = pd.read_csv('ubicaciones_el_salvador.csv', sep=';', encoding='latin1')
...
if menu == "Ver Datos":
    ...
    # 🌟 Aquí está el botón que solicita. Solo se muestra si la tabla no existe.
...
# --- 🚚 Predicción de Rutas simuladas ---
    ...
    if uploaded_file is not None:
        try:
            # 🐛 Solución: Se especifica la codificación 'latin1'
            ubicaciones_df = pd.read_csv(uploaded_file, encoding='latin1')
